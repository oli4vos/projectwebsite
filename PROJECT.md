...
Voor toeslagen blijft `evaluateAllowanceSignals` de centrale hard-check- en eligibilitylaag voor bekende 2026-voorwaarden. `evaluateAllowanceRegulations` verrijkt die uitkomst met Regulations-contracten, unknown resolution, inference, recommendations en reportcontext. De publieke Toeslagenscan gebruikt centrale adapters en officiële 2026-engines voor bedragindicaties waar de regels volledig genoeg zijn genormaliseerd: zorgtoeslag via de bestaande scanroute, huurtoeslag via `calculateRentBenefit2026`, kindgebonden budget via `calculateChildBudget2026` en kinderopvangtoeslag via `calculateChildcareBenefit2026`. Complexe of onvolledige kinderopvangsituaties blijven geblokkeerd of incompleet.
